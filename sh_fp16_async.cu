...
void attention(
    ...
    cuda_status = cudaMalloc((void**)&d_q, size_qkv);
    cuda_status = cudaMalloc((void**)&d_k, size_qkv);
    cuda_status = cudaMalloc((void**)&d_v, size_qkv);
    cuda_status = cudaMalloc((void**)&d_mask, size_mask);
    cuda_status = cudaMalloc((void**)&d_output, size_qkv);
    cuda_status = cudaMemcpyAsync(d_q, q, size_qkv, cudaMemcpyHostToDevice, stream);
    cuda_status = cudaMemcpyAsync(d_k, k, size_qkv, cudaMemcpyHostToDevice, stream);
    cuda_status = cudaMemcpyAsync(d_v, v, size_qkv, cudaMemcpyHostToDevice, stream);
    cuda_status = cudaMemcpyAsync(d_mask, mask, size_mask, cudaMemcpyHostToDevice, stream);
    ...
    cuda_status = cudaMemcpyAsync(output, d_output, size_qkv, cudaMemcpyDeviceToHost, stream);
